...
class Part extends Model { 
    ...
    public static function get_part_vnumbers(Int $part_id, Int $station_id, Int $org_id){
        ...
        $results = DB::select("SELECT pv.v_num, CONCAT(pv.v_num, IF(cv.id IS NULL, '', IF(cv.is_completed = 0, ' (In Progress)', IF(cv.compliance_ind = 1, ' (Inspected - Compliant)', ' (Inspected - Non Compliant)')))) AS v_num_label
