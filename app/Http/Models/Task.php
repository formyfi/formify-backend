...
class Task extends Model { 
    ...
    public static function get_task_list(Int $org_id, Int $user_id){
        ...
        $list = DB::select("SELECT CONCAT(u.first_name, ' ' ,u.last_name) AS last_updated_by_name, cv.part_id, cv.station_id, cv.vnum_id, cv.form_id, IF(cv.compliance_ind = 1, 'Yes', 'No') AS compliance_ind, cd.form_data, fs.form_json, cd.last_updated_id, s.name AS station_name, p.name AS part_name
            ...
            WHERE cv.org_id = ? AND cv.station_id IN (SELECT GROUP_CONCAT(DISTINCT station_id) FROM user_station WHERE user_id = ?)", [$org_id, $user_id]);
